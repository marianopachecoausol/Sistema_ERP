...
Select Case Index
   ...
   Case 4
      If Command1(Index).Caption = "&Agregar" Then
         ...
         If Not mRec.EOF Then
            mRec.MoveLast
            Codigo = mRec!Codigo
            Text1(0).Text = Format((Codigo + 1), "0000")
            Text1(1).Enabled = True
            Text1(1).Text = ""
            Text1(1).SetFocus
         Else
   ...
   Case 6
      ...
      If mTabla = "Almacenes" Then
      
      Set mRec1 = mObj.oTabla("Bodegas", "WHERE CodAlmacen = '" & Text1(0).Text & "' AND Fecha_Baja IS NULL")
      ...
      If bResult = True Then
         ...
         End If
      Else
         MsgBox "NO puede eliminar este Almacén!!! " & vbCrLf & "Aún tiene Bodegas asociadas!!!", vbCritical, sMessage
      End If
   Case 7
   ...
   Case 8
      ...
      Set mRec1 = mObj.oTabla(mTabla, "order by codigo asc")
      If mTabla = "otros" Then
         ...
         Do While Not mRec1.EOF
            mData.Execute "insert into Reportes (codigo,descripcion,CodTipoOtro) values ('" & mRec1.Fields(0) & "','" & mRec1.Fields(1) & "','" & mRec1.Fields(2) & "'"
            mRec1.MoveNext
         Loop
      Else
         mData.Execute ("CREATE TABLE Reportes (Codigo TEXT,Descripcion TEXT, Fecha_Baja TEXT)")
         Do While Not mRec1.EOF
            mData.Execute "insert into Reportes (codigo,Descripcion,Fecha_Baja) values ('" & mRec1.Fields(0) & "','" & mRec1.Fields(1) & "','" & mRec1.Fields(2) & "')"
            mRec1.MoveNext
         Loop
      End If
      mRec1.Close
...
Inven2_frm.Height = 4695
Inven2_frm.Width = 9420
sAlinearForm Me
For mi = 0 To Text1.UBound
   Text1(mi).Text = ""
Next
Set mRec = mObj.oTablaDina(mTabla, " where Fecha_Baja IS NULL order by Codigo ")
If Not mRec.EOF Then
   Reporte = "Rep01.rpt"
   If mTabla = "Bodegas" Then
      Set mRecB = mObj.oTablaNull("Almacenes")
