...
Begin VB.Form MantElect12 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Abastecimiento O.T. - Ajustes"
   ClientHeight    =   9405
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   16965
...
Private Sub Combo3_Click()
   ...
   Set mRec = mObj.oEjecutarSelect("SELECT O.NroVale, O.CodTipoVale,O.CodUbicacion, U.Descripcion AS Ubicacion " & _
   " FROM OT_Abastecimiento_H O " & _
   " Inner Join " & _
   " Inventario.Ubicaciones U ON U.Codigo = O.CodUbicacion " & _
   " WHERE O.IdOT = " & mIdOT & ";")
...
Private Sub llenoGrillaAjustes(pIdOT As Integer, pCodUbicacion As String)
 ...
 'Elimino los registros  de la grilla
  For mi = FlexProduct.Rows To 3 Step -1
   ...
   'Cargo la Grilla INFERIOR con lo devuelto por el sp "getConsumoAbastecimientoXidOTyUbicacion"
   If Not mRec.EOF Then
      mi = 1
      Do While Not mRec.EOF
         mi = mi + 1
...
Private Sub Command1_Click()
   ...
   If Combo3.ListIndex <> -1 Then
      ...
      sMsgEspere Me, "", False
   Else
       MsgBox "Debe seleccionar alguna Orden de Trabajo para poder buscar Productos.", vbExclamation, "Buscar Productos"
   End If
End Sub
...
Private Sub Command2_Click(Index As Integer)
   ...
   If Index = 0 Then
      If fValidaConfirmarAjuste() Then
         ...
         mObj.xAjustarAbasteciemientoOT IdOT, vMat_CodProd_Ajuste(), vMat_CodUbic_Ajuste(), vMat_Cantidad_Ajuste(), vMat_CantidadBD_Ajuste(), _
...
Private Sub Form_Load()
   ...
   Set mRec = mObj.oEjecutarSelect("SELECT CONVERT( CONCAT(LPAD(O.IdOT,10,'0'),' - ',Date_Format(O.Fecha,'%d/%m/%Y')),char(23)) AS OT_Fecha, O.Fecha " & _
                           " FROM MantElect.OT_H O " & _
                           " Inner Join " & _
                           " OT_Abastecimiento_H A ON O.IdOT = A.IdOT " & _
